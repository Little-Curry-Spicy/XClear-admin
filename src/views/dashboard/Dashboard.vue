  ...
  <div class="space-y-6">
      ...
      <Card v-for="stat in stats" :key="stat.title">
        <template #header>
          <div class="flex items-center justify-between p-4">
            ...
            <i :class="stat.icon" class="text-muted-foreground text-lg" />
          </div>
        </template>
        <template #content>
    ...
    <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
      <Panel class="col-span-4">
        <template #header>
          <span class="font-semibold">收入概览</span>
        </template>
        <div class="h-[300px] w-full">
          <v-chart class="h-full w-full" :option="revenueChartOption" autoresize />
        </div>
      </Panel>
      <Panel class="lg:col-span-3 md:col-span-4 col-span-4">
        <template #header>
        ...
        <div class="space-y-4">
          ...
          >
            ...
            >
              <i :class="[activity.icon, activity.iconColor]" class="text-lg" />
            </div>
            <div class="flex-1">
    ...
    <Panel>
      <template #header>
        <span class="font-semibold">分类销售统计</span>
      </template>
      <div class="h-[280px] w-full">
        <v-chart class="h-full w-full" :option="categoryChartOption" autoresize />
      </div>
    </Panel>
  </div>
</template>
...
const stats = [
  { title: '总营收', value: '¥45,231', change: '+20.1% 较上月', icon: 'pi pi-dollar' },
  { title: '订阅用户', value: '+2,350', change: '+180.1% 较上月', icon: 'pi pi-users' },
  { title: '销售额', value: '+12,234', change: '+19% 较上月', icon: 'pi pi-shopping-cart' },
  { title: '活跃用户', value: '+573', change: '+201 较上月', icon: 'pi pi-chart-line' },
]
